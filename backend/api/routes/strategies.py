...
# Optional Gemini strategist (compatible with user's latest file)
_GEMINI_AVAILABLE = False
try:
    from ...ai_services.gemini_strategist import GeminiStrategist  # type: ignore
...
def _augment_models(selected: List[str], cap: int = 8) -> List[str]:
    ...
    if len(selected) >= 3:
        return selected[:cap]
    # add one per category, then fill
...
async def _try_gemini_generate(dataset_id: str, objective: Optional[str]) -> tuple[Dict[str, Any], str]:
    if not _GEMINI_AVAILABLE:
        raise ValueError("Gemini strategist not available")

    strat = GeminiStrategist()
    context = {
        "dataset_id": dataset_id,
        "objective": objective or "Optimize clinical risk and anomaly detection with explainable outputs.",
        "medical_context": {"specialty": "general"},
        "available_models": _all_catalog_models(),
        "model_categories": {
            "readmission": STRATEGIES[0]["models"],
            "critical_care": STRATEGIES[1]["models"],
            "chronic_disease": STRATEGIES[2]["models"],
            "operational": STRATEGIES[3]["models"],
        },
    }
    plan = strat.generate_comprehensive_strategy(context)
    if hasattr(plan, "__await__"):
        plan = await plan
    if not isinstance(plan, dict):
        raise ValueError("Gemini returned non-dict plan")

    # Extract models
    selected: List[str] = []
    exec_plan = plan.get("model_execution_plan", {})
    for key in ["primary_models","models","selected_models","recommended_models"]:
        block = exec_plan.get(key)
        if isinstance(block, list):
            for item in block:
                name = item.get("model_name") if isinstance(item, dict) else str(item)
                if name:
                    selected.append(name)
    if not selected:
        for key in ["models","selected_models","recommended_models"]:
            block = plan.get(key)
            if isinstance(block, list):
                for item in block:
                    name = item.get("model_name") if isinstance(item, dict) else str(item)
    ...
    selected = _augment_models(selected)
    thresholds = exec_plan.get("thresholds") or {m: _DEFAULT_THRESHOLDS.get(m, 0.5) for m in selected}

    parsed = {
        "version": "1.0",
        "selected_models": selected,
        "thresholds": thresholds,
        "post_processing": {"calibration": exec_plan.get("calibration", {}).get("method","none")},
        "validation_plan": exec_plan.get("validation_plan", {
            "cv_folds": 3, "metrics": {"classification":["roc_auc","pr_auc","f1","recall"], "regression":["mae","r2"]}
        }),
        "visualization_specifications": plan.get("visualization_specifications", {
        ...
        "confidence_score": plan.get("confidence_score", 0.7),
        "medical_specialty_focus": plan.get("medical_specialty_focus", "general"),
    }
    rationale = f"Gemini strategist used. Confidence={parsed['confidence_score']}. Models selected: {len(selected)}."
