...
async def upload_dataset(
    ...
    try:
        ...
        _ = _insert_records(eng, dataset_id, df)
    ...
    # 1) Create & persist a strategy (await! it's async)
    try:
        strategy = await create_and_store_strategy(
            dataset_id=str(dataset_id),
            preset_id=None,
            objective="Optimize clinical risk & anomaly detection with explainable outputs."
        )
    except Exception as e:
        strategy = {
            "id": None,
            "dataset_id": dataset_id,
            "raw_text": f"Strategy creation failed: {e}",
            "parsed": {
                "version": "1.0",
                "selected_models": [],
                "thresholds": {},
    ...
    # 2) Run predictions/anomaly detection with strategy
    try:
        analysis_result = _run_predictions_for_strategy(_get_engine(), dataset_id, strategy.get("parsed", {}))
    except Exception as e:
        analysis_result = {"summary": f"analysis failed: {e}", "exports": {}}

    return {
        "dataset": {
            "id": dataset_id,                 # << canonical ID used by GET /datasets/{id}
            "name": safe_name,
            "n_rows": n_rows,
            "n_cols": n_cols,
            "ingest": "postgres",
